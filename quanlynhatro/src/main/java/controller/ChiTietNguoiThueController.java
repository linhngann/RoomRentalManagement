...
public class ChiTietNguoiThueController implements Action{
	...
	public static void updateNguoiThue(ChiTietNguoiThue view) {
		...
		nguoiThue.setIdnguoiThue(idNguoiThue);
		...
		//JOptionPane.showMessageDialog(null,view.getButtonRadioGroupGioiTinh().getSelection().getActionCommand() );
		if( view.getButtonRadioGroupGioiTinh().getSelection() != null) {
			if(view.getRadioButtonNam().isSelected()) {
				nguoiThue.setGioiTinh("Nam");
			} else {
				nguoiThue.setGioiTinh("Nữ");
			}
		
		}
		if(view.getchckbxTrangThai().isSelected()) {
			nguoiThue.setTrangThai('Y');
		}else {
			nguoiThue.setTrangThai('N');
		}
		...
		if(update_tt) {
			...
			JOptionPane.showMessageDialog(null,"Đã chỉnh sửa người thuê " + nguoiThueDAO.getInstance().update(nguoiThue));
		}
	}
